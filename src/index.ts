...
server.registerTool(
  "media_stack_overview",
  {
    ...
    description: "Return a compact dashboard of service status, health, queues, missing media, disk space, indexers, library counts, and import issues.",
  },
  async () => {
    try {
...
server.registerTool(
  "service_status",
  {
    ...
    description: "Check reachability, auth, version, and warning summary for one service or all configured services.",
    inputSchema: {
...
server.registerTool(
  "download_queue",
  {
    ...
    description: "Return normalized queue items across Sonarr, Radarr, Lidarr, and SABnzbd.",
    inputSchema: {
...
server.registerTool(
  "recent_activity",
  {
    ...
    description: "Return normalized recent activity/history across configured services.",
    inputSchema: {
      service: appName.optional(),
...
server.registerTool(
  "get_missing_summary",
  {
    ...
    description: "Return missing wanted counts and samples for Sonarr, Radarr, and Lidarr.",
    inputSchema: {
...
server.registerTool(
  "indexer_status",
  {
    ...
    description: "Return Prowlarr indexer configuration, failure status, and health issues without credentials.",
  },
  async () => {
    try {
...
server.registerTool(
  "get_library_counts",
  {
    ...
    description: "Return Sonarr series, Radarr movie, Lidarr artist, and Lidarr album counts.",
  },
  async () => {
    try {
...
server.registerTool(
  ...
  {
    title: "Import Issues",
    description: "Return queue/import warnings and failed recent history items across media services.",
    inputSchema: {
      pageSize: z.number().int().min(1).max(100).default(50),
    },
  },
